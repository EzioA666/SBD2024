...
let parse_term : symbol parser = (* TODO *)
  fun cs ->
    let rec collect_chars acc = function
      | '\'' :: rest -> Some (T (implode (List.rev acc)), rest)  (* Close quote: Return terminal symbol *)
      | c :: cs -> collect_chars (c :: acc) cs  (* Collect chars inside quotes *)
      | [] -> None  (* Unmatched quote: parsing fails *)
    in
    match cs with
    | '\'' :: cs -> collect_chars [] cs  (* Opening quote found, start collecting *)
    | _ -> None  (* Does not start with a quote: Not a terminal symbol *)
...
fun input -> 
  let rec parse_id acc = function
    | [] -> None (* No closing '>' found, invalid input *)
    | '>' :: cs' when acc <> [] && List.hd acc <> '-' -> 
        (* Correctly construct an NT symbol with the accumulated identifier *)
        Some (NT (implode (List.rev acc)), cs')
    | c :: cs' when ('a' <= c && c <= 'z') || (c = '-' && acc <> [] && List.hd acc <> '-') -> 
        parse_id (c :: acc) cs'
    | _ -> None (* Invalid character sequence *)
  in
  match input with
  | '<' :: cs -> parse_id [] cs
  | _ -> None
